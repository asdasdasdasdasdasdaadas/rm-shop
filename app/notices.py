...
DEFAULT_NOTICES: dict[str, str] = {
    ...
    "welcome_intro_try": (
        ...
        "Дальше всё из кабинета, не из переписки."
    ),
    "welcome_intro_try_no_trial": (
        "Открой кабинет и добавь устройство — займёт минуту. "
        "Дальше всё из приложения: ссылка, устройства, поддержка — без переписки с ботом."
    ),
    ...
    "trial_nudge": (
        ...
        "Кабинет в Telegram, платите только за свои устройства. "
        "Если не зайдёт — просто не продлевайте."
    ),
    "invite_nudge": (
        "{name}, за друга можно получить {reward}.\n\n"
        "{when} "
        "Друг попадёт в тот же кабинет в Telegram, не на чужой сайт.\n\n"
        "Ваша ссылка:\n{link}"
    ),
    "broadcast_invite": (
    ...
    "broadcast_unused": (
        ...
        "Здесь не общий ключ из чата: кабинет в Telegram и списание только за добавленные устройства. "
        "Откройте кабинет и добавьте одно. Пока устройств нет, баланс не списывается."
    ),
    "info_nudge": (
        "Как устроен кабинет.\n\n"
        ...
        "Поддержка тоже здесь: не нужно искать другой сервис, когда Telegram уже не открывается."
        "{story}"
    ),
    "story_nudge": (
    ...
    "first_device_thanks": (
        ...
        "Если держит — удобно остаться: ссылку можно перевыпустить в кабинете, "
        "поддержка тоже там."
    ),
    "first_online_nudge": (
    ...
    "device_nudge_1": (
        ...
        "Без него VPN не включится — это не общий ключ из чата, а ваше устройство в кабинете. "
        "Откройте кабинет — займёт меньше минуты."
    ),
    "device_nudge_2": (
    ...
    "idle_nudge_7": (
        "Неделю VPN не подключался. Если что-то сломалось — напишите в поддержку из кабинета, разберёмся.\n\n"
        "Кабинет открывается из бота даже без VPN. Можно пополнить баланс или проверить устройство."
    ),
    "idle_nudge_10": (
        "Уже десять дней без подключения. Если VPN всё ещё нужен, откройте кабинет и проверьте устройство.\n\n"
        "Кабинет в боте, даже когда сеть недоступна. Ссылку можно перевыпустить там же."
    ),
    "idle_nudge_15": (
        "Пятнадцать дней без онлайна. Кабинет на месте: баланс, устройства и поддержка в Telegram.\n\n"
        "Если понадобится снова — откройте кабинет из бота и подключитесь."
    ),
    "idle_nudge_20": (
        "Двадцать дней без подключения. Это последнее напоминание на этой паузе.\n\n"
        "Кабинет останется в боте. Когда понадобится VPN — зайдите и подключитесь снова."
    ),
