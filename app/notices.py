...
DEFAULT_NOTICES: dict[str, str] = {
    ...
    "device_nudge_1": (
        ...
        "Без него VPN не включится. Нажмите «Открыть кабинет» — займёт меньше минуты."
    ),
    "device_nudge_2": (
        "Напоминаем: устройство всё ещё не добавлено. "
        "Пока его нет, VPN не работает, баланс не списывается. "
        "Нажмите «Открыть кабинет» и подключитесь."
    ),
    "device_nudge_3": (
        "Последнее напоминание: без устройства VPN не запустится. "
        "Нажмите «Открыть кабинет», когда будете готовы."
    ),
    "idle_nudge_7": (
