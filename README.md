...
Кнопка «Продолжить оплату» открывает выбор суммы в кабинете, а не старый счёт. Настройка «Незавершённая оплата» в админке включена по умолчанию; текст доступен в редакторе сообщений. Повторная ручная отправка старого напоминания из журнала отключена. Начало отсчёта — создание счёта, а не простой просмотр экрана оплаты.
