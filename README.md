...
Возможности: обзор, пиксельный офис команды, пользователи (начислить дни, сброс бесплатного периода, личное сообщение), заказы RollyPay, жалобы VPN, рассылка, бэкапы, тех. работы (сохранённые текст и картинка на любое действие в боте), просмотр настроек без секретов.

### Офис команды

Вкладка «Офис» (`/admin#office`) рисует 2D-зал с четырьмя столами. Состав фиксированный: Понедельник (код), Пятница (Threads/PR), Четверг (метрики), Продакт (QA/деплой). Чтение статусов только с cookie админки. Запись — отдельным секретом, без браузерной сессии.

В `.env`:

```
OFFICE_STATUS_TOKEN=длинный-секрет
OFFICE_STALE_MINUTES=8
```

Агент шлёт пульс:

```bash
curl -sS -X POST http://127.0.0.1:8080/api/office/status \
  -H "X-Office-Token: $OFFICE_STATUS_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"agent_id":"monday","state":"busy","message":"пишет кабинет"}'
```

`agent_id`: `monday` / `friday` / `thursday` / `product` (или русские имена). `state`: `busy`, `idle`, `offline` (`active` = занят). Поле `ts` необязательно. Без апдейта дольше `OFFICE_STALE_MINUTES` стол гаснет. Статусы лежат в таблице `office_agent_status` и переживают рестарт бота.
...
| `OFFICE_STATUS_TOKEN` | Секрет `POST /api/office/status` для агентов офиса |
| `OFFICE_STALE_MINUTES` | Минут без пульса, после которых стол офлайн (по умолчанию 8) |
...
| `/admin` | админка |
| `/api/office/status` | пульс агентов офиса (токен `OFFICE_STATUS_TOKEN`) |
