...
Дамп PostgreSQL каждый день в **00:01 по Москве** и по кнопке в админке (вкладка «Бэкапы»). Файл `.sql.gz` пишется в `backups/` на сервере и уходит в Telegram всем из `ADMIN_IDS`. Старые файлы чистятся через `BACKUP_KEEP_DAYS` (по умолчанию 14, последние 7 файлов не трогаем).
...
- Бэкап БД: каждый день в 00:01 МСК и вручную из админки. Копия — в `backups/` и админам в Telegram.
