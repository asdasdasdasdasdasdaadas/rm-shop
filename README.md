...
Telegram-бот магазина VPN на [Remnawave](https://docs.rw/). Пользователь подписывается на канал, нажимает «Попробовать бесплатно» или покупает подписку через RollyPay. Срок и ссылка подключения создаются в панели Remnawave. Данные пользователей хранятся в PostgreSQL.
...
1. Обязательная подписка на канал. Уведомление со ссылками на оферту и политику встроено в приветствие и профиль.
...
7. Живой канал из админки (Настройки → Бренд): оплатил, пригласил, продлил, выложил историю, подключился первый раз.
...
2. Приветствие и профиль содержат ссылки на оферту и политику без отдельного экрана принятия. После успешной отправки уведомления сохраняется `legal_notice_at`; последующее действие в боте или открытие кабинета (`/api/me`) фиксирует `accepted_legal_at` один раз. Сам `/start` не фиксирует принятие. Старые кнопки «Принимаю» продолжают работать, напоминания о принятии больше не отправляются.

   Перед выкладкой обновите опубликованную оферту по `LEGAL_OFFER_URL`: локальный шаблон `legal/offer.txt` не заменяет документ по внешней ссылке.
3. После приветствия бот показывает полное меню и сообщает о подарке в личном кабинете. Для получения подарка обязателен `/start` в боте: сохраняется `bot_started_at`. При прямом входе в Web App экран подарка предлагает «Запустить бота», а `/api/trial` отклоняет начисление до запуска. При `/start`, открытии профиля и загрузке кабинета подарок не начисляется. На приветственном экране кнопка «Принять подарок» вызывает `/api/trial`: начисляет тестовый баланс один раз и открывает добавление устройства. Ранее начисленные подарки не выдаются повторно.
...
| --- | --- | --- | --- |
| Пригласивший | дни подписки или, при балансе, рубли | `REFERRAL_REWARD_DAYS` / `REFERRAL_REWARD_RUB` | 7 дней / 50 рублей |
| Друг по ссылке | плюс к бесплатному периоду или, при балансе, те же рубли | `REFERRAL_INVITEE_DAYS` / `REFERRAL_REWARD_RUB` | 5 дней / 50 рублей |
...
Возможности: обзор, пользователи (начислить дни, сброс бесплатного периода, личное сообщение), заказы RollyPay, жалобы VPN, рассылка, бэкапы, тех. работы (сохранённые текст и картинка на любое действие в боте), просмотр настроек без секретов.

## Бэкапы

Дамп PostgreSQL каждый день в **00:01 по Москве** и по кнопке в админке (вкладка «Бэкапы»). Файл `.sql.gz` пишется в `backups/` на сервере. Старые файлы чистятся через `BACKUP_KEEP_DAYS` (по умолчанию 14, последние 7 файлов не трогаем).

Переезд на другой хост: на новом сервере поднимите бота, откройте админку → «Бэкапы» → загрузите `.sql` / `.sql.gz`. Текущая база заменится. Токен, домен и ключи кассы остаются в `.env` нового хоста.

Восстановление (осторожно, перезапишет данные):

```bash
gunzip -c backups/rm-shop-YYYY-MM-DDTHHMM-MSK.sql.gz | docker compose exec -T postgres psql -U "$POSTGRES_USER" -d "$POSTGRES_DB"
```
...
| `ADMIN_LIVE_CHAT_ID` | Запасной id канала живых событий, если в админке поле ещё пустое. Основная настройка: Настройки → Бренд |
...
| `BACKUP_KEEP_DAYS` | `14` | Сколько дней хранить файлы бэкапов |
...
- Живой канал: в админке Настройки → Бренд. Публичный — `@username`. Приватный — бот админ с правом публиковать, id `-100...` или ссылка `t.me/c/...` (не `t.me/+`). После добавления бота в канал в форме появится «Подставить id». Кто уже был в онлайне до включения, в канал не пишется.
...
**В живой канал ничего не пишется.** В админке: Настройки → Бренд. Для приватного канала пригласительная ссылка не работает: сделайте бота админом, обновите настройки — появится кнопка «Подставить id». Нужно право публиковать сообщения. Проверка историй по-прежнему в `ADMIN_IDS`.
