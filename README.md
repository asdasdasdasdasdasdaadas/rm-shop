...
7. Живой канал из админки (Настройки → Бренд): оплатил, пригласил, продлил, подключился первый раз.
...
**В живой канал ничего не пишется.** В админке: Настройки → Бренд. Для приватного канала пригласительная ссылка не работает: сделайте бота админом, обновите настройки — появится кнопка «Подставить id». Нужно право публиковать сообщения.
...
После успешного создания платёжной ссылки или счёта Stars сохраняется последняя попытка оплаты пользователя. Одно напоминание отправляется через 10 минут (плюс до 2 минут на цикл фоновой задачи), не чаще раза за 24 часа. Попытки старше суток, пользователи без `/start` и заблокировавшие бота исключаются. Успешное зачисление отменяет ожидание. Для RollyPay перед отправкой проверяется статус: оплаченные и возвращённые платежи исключаются; при ошибке проверки отправка откладывается. Для отменённых и истёкших счетов кнопка ведёт к созданию нового платежа. Stars дополнительно проверяются по таблице подтверждённых платежей.

Кнопка «Продолжить оплату» повторно проверяет последний счёт пользователя: до 20 минут возвращает его платёжную ссылку, после истечения предлагает создать новый счёт в кабинете. Оплаченный счёт или платёж в обработке не предлагает оплатить повторно; при недоступности провайдера просит повторить проверку позже. Ссылки Stars также сохраняются. Настройка «Незавершённая оплата» в админке включена по умолчанию; текст доступен в редакторе сообщений. Повторная ручная отправка старого напоминания из журнала отключена. Начало отсчёта — создание счёта, а не простой просмотр экрана оплаты.


## Напоминания после подарка

После получения подарка сохраняется `gift_claimed_at`. Через 30 минут без первого подключения (`first_online_at`) отправляется одно напоминание с кнопками подключения и помощи — в том числе если устройство уже добавлено. Старые пользователи без даты получения подарка не попадают в новую рассылку задним числом. Текст редактируется в админке: «Подарок без подключения: 30 минут».

При положительном подарочном балансе не больше суточного расхода отправляется предупреждение с примерным остатком часов и кнопкой пополнения. Расход учитывает все обычные устройства, исключает роутеры; при общей или пользовательской паузе списаний предупреждение не отправляется. После первого платного пополнения этот сценарий не применяется. Текст «Заканчивается подарок» поддерживает `{hours}` и `{devices}`. Ранее сохранённые в админке тексты сохраняют приоритет над стандартными.

Сначала проверяется незавершённая оплата, затем окончание подарка, подключение и прочие напоминания. Во время 20-минутного окна оплаты остальные сценарии откладываются. История успешных напоминаний, рассылок и предупреждений о нулевом балансе даёт паузу 24 часа для обычных сценариев и 6 часов для окончания подарка; ограничение переживает перезапуск. Напоминание об оплате имеет приоритет и собственный суточный лимит. Сервисные сообщения об оплате и ответы на действия пользователя не откладываются.


Публикация историй за награду удалена: предложений, напоминаний и модерации больше нет. Старый API публикации возвращает 410, начисление наград отключено. Ранее начисленный баланс и его история сохранены.


## Реферальная акция: 50 ₽ + 5%

В режиме баланса пригласивший получает 50 ₽ однократно за первую оплату нового друга и 5% с каждого оплаченного пополнения, включая первое. 300 ₽ впервые → 65 ₽ награды, повторно → 15 ₽. Проценты считаются с суммы пополнения VPN-баланса (включая рублёвый эквивалент Stars), а не с подарков, промокодов, административных начислений или оплаты роутера. Остатки процентов меньше рубля накапливаются до целого рубля.

Админка → Рефералы → «Начислять реферальные награды» → Сохранить. По умолчанию выключено. Пауза видна в кабинете и боте; автоматические реферальные приглашения отключены. Платежи во время паузы записываются без награды, после включения не пересчитываются. Если первая оплата друга была в паузу, следующие дают только 5%, без отложенного бонуса 50 ₽. Ранее начисленные средства и возможность вывода по текущим правилам сохраняются. Отдельная существующая настройка бонуса самому другу сохранена и тоже подчиняется общему переключателю.

Начисление пригласившему, перенос дробного остатка и журнал проводок выполняются одной транзакцией. Идентификатор платежа RollyPay/Stars уникален в `referral_payment_rewards`, повторная обработка не выдаёт награду ещё раз.
