...
- Живой канал: в админке Настройки → Бренд. Бот должен быть администратором канала. Кто уже был в онлайне до включения, в канал не пишется.
...
**В живой канал ничего не пишется.** В админке: Настройки → Бренд → канал живых событий (`@channel` или `-100...`). Бот — админ канала. Проверка заявок на историю по-прежнему уходит в `ADMIN_IDS`.
